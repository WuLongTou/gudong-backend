...
impl SessionCacheOperations {
    ...
    pub async fn cache_session(
        ...
        let json = serde_json::to_string(&cached_session).map_err(|e| {
    ...
    pub async fn get_session(
        ...
        match result {
            Some(json) => {
                let cached_session = serde_json::from_str(&json).map_err(|e| {
    ...
    pub async fn refresh_session(
        ...
        match Self::get_session(redis, session_id).await? {
            Some(session) => {
                Self::cache_session(redis, session_id, &session.user_id, session.data, ttl).await?;
                Ok(())
            }
