...
use crate::cache::models::activity::{CachedNearbyUser, CachedUserActivity, NearbyUser, UserActivity};
use crate::cache::keys::{ACTIVITY_GEO_KEY, USER_GEO_KEY, user_cache_key, activity_cache_key};
...
impl ActivityCacheOperations {
        ...
        match users_result {
            Ok(users) => {
                for (user_id, distance) in users {
                    // 获取用户详情
                    let user_data: redis::RedisResult<String> = conn.get(user_cache_key(&user_id)).await;
                    
                    if let Ok(user_data) = user_data {
                        if let Ok(mut cached_user) = serde_json::from_str::<CachedNearbyUser>(&user_data) {
                            // 更新距离
    ...
    /// 缓存附近活动
    pub async fn cache_nearby_activities(&self, activities: &Vec<UserActivity>) -> redis::RedisResult<()> {
        let mut conn = self.redis_client.get_multiplexed_async_connection().await?;
        ...
        match activities_result {
            Ok(activities) => {
                for (activity_id, distance) in activities {
                    // 获取活动详情
                    let activity_data: redis::RedisResult<String> = conn.get(activity_cache_key(&activity_id)).await;
                    
                    if let Ok(activity_data) = activity_data {
                        if let Ok(mut cached_activity) = serde_json::from_str::<CachedUserActivity>(&activity_data) {
                            // 更新距离
